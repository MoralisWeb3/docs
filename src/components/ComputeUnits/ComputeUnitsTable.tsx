...
import endpoints from "@site/configs/api-reference/compute-units.json";
import camelToSnakeCase from "../../../utils/camelToSnakeCase.mts";
...
const fixedPath = {
    ...
    getNFTHistoricalFloorPriceByContract: "get-nft-historical-floor-price-by-contract",
    ...
    getVolumeStatsByChain: "get-volume-stats-by-chain",
    getVolumeStatsByCategory: "get-volume-stats-by-category",
    getTimeSeriesVolume: "get-time-series-volume",
            ...
            <tbody>
                {endpoints?.[chain]?.map((e: EndpointWeightResponse, index: number) => {
                    ...
                    const additionalInfo = dynamic
                        ? `+${price} CU${price > 1 ? "s" : ""} per ${DynamicPriceUnits?.[endpoint]}`
                        : "0";
