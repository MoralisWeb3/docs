...
import { ChartTimezone, FormattedTimezone, getFormattedTimezone, PRICE_CHART_TIMEZONES } from '../utils/timezone';
...
export const useFormattedTimezones = () => {
  return useMemo(() => {
    const formatted = PRICE_CHART_TIMEZONES.map(getFormattedTimezone)
      .filter((tz): tz is { offset: string; cityName: string; timezone: ChartTimezone } => tz !== null)
      .sort((a, b) => {
