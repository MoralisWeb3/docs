...
export default function AssistantBot() {
    ...
    async function fetchMessages() {
        ...
        try {
            const response = await fetch(`/api/moralis-assistant-messages?thread_id=${threadId}`);
            if (!response.ok) {
                throw new Error("Network response was not ok");
            }
    ...
    return (
        <>
                                    ...
                                    <div className="flex items-start w-5/6">
                                        ...
                                        <div className="ml-4 bg-slate-300 dark:bg-slate-600 p-2 rounded-lg">
                                            <p className="text-sm dark:text-orange-400 mb-0">
                                                <ReactMarkdown remarkPlugins={[remarkGfm]}>
                                                    Error occured in answering. Please refresh the
            ...
            <EmailInputDialog isOpen={open && isEmailDialogOpen} onClose={closeEmailDialog} />
        </>
    );
}
