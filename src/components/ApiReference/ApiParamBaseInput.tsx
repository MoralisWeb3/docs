...
const ApiParamBaseInput = ({
    ...
    const fieldProps = {
        ...field,
        value: valueToString ? (field.value ? valueToString(field.value) : "") : field.value ?? "",
    ...
    return (
        ...
        >
            {options ? (
                <select {...fieldProps} className={inputClassName} disabled={param?.disabled}>
                    <option />
                ...
                <input {...fieldProps} className={inputClassName} disabled={param?.disabled} />
            )}
            {focused && !param?.disabled && (
                <ApiParamInputOverlay field={field} form={form} meta={meta} param={param} />
            )}
        </div>
    );
};
